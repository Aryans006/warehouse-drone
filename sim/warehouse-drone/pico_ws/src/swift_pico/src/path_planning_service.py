...
class BidirectionalAStar:
    def __init__(self, s_start, s_goal, heuristic_type):
        ...
        self.Env = Env("/home/insaanimanav/eyantra/warehouse-drone/pico_ws/src/swift_pico/src/image.webp")  # class Env
...
class WayPoints(Node):
    ...
    def __init__(self):
        ...
        self.counter = 0
        self.drone_position = (500.0, 500.0)
        self.start = (0.0, 0.0)
        self.end = (0.0, 0.0)
    ...
    def path1_callback(self, request, response):
        ...
        if self.counter == 0:
            bastar = BidirectionalAStar(self.drone_position[:2], self.start[:2], "euclidean")
            ...
            if request.get_path1:
                ...
                print("selfend", self.pixel_to_whycon(self.start[0]))
                print("selfend", self.pixel_to_whycon(self.start[1]))
                # Append your final pose
                custom_pose = Pose(
                    position=Point(
                        x=self.pixel_to_whycon(self.start[0]),
                        y=self.pixel_to_whycon2(self.start[1]),
                        z=27.0
                    ),
                    orientation=Quaternion(x=0.0, y=0.0, z=0.0, w=1.0)
                )
            ...
            bastar = BidirectionalAStar(self.start[:2], self.end[:2], "euclidean")
            path, visited_fore, visited_back = bastar.searching()
            self.counter = 0
            ...
            if request.get_path1:
                ...
                print("selfend", self.pixel_to_whycon(self.end[0]))
                print("selfend", self.pixel_to_whycon2(self.end[1]))
                custom_pose_4 = Pose(
                    position=Point(
                        
                        x=self.pixel_to_whycon(self.end[0]),
                        y=self.pixel_to_whycon2(self.end[1]),
                        z=27.0
                    ),
                    orientation=Quaternion(x=0.0, y=0.0, z=0.0, w=1.0)
                )
    ...
    #     elif self.counter == 2:
    #         bastar = BidirectionalAStar(self.path_array[1], self.path_array[2], "euclidean")
    #         path, visited_fore, visited_back = bastar.searching()
    #         self.counter = 0
    #         print("this is path 3", path)
    #         if request.get_path1 == True :
    #             response.plannedpath1.poses = [
    #                 Pose(
    #                     position= Point(
    #                         x=self.pixel_to_whycon(path[i][0]),  
...
##CHANGE PATH ARRAY FOR THIRD PAATHS